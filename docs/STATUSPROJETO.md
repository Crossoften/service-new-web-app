...
>
> **Base de código:** `origin/main` @ `f4931bf`. **Branch de entrega (cliente):** `integracao` (@ `947c73d`).
...
> **Próximas fatias sugeridas (🟢, back pronto):** **CART-1** (editar sacola: quantidade/remover) · busca/filtros de
> restaurante · "pedir novamente" · avaliar pós-entrega. Cupom/agendamento/gorjeta/push exigem back novo → `backend-demandas.md`.
...
Pendente de aplicação: **CV-3** (indep.), **AJ-layout** (2 `.scss`), **AJ-maps-key** (chave do Google) e **OBS-1** (observação por item). *(ADD-1/ADD-2 aplicados; base @ `947c73d`.)*
`origin/integracao` @ `947c73d`.
