...
| **UX-CHAT-REAL (Fatia 4)** | **(a)** Botão de **chat real** no header de `detalhes-trabalho` (fornecedor) → `/chat/:chatId` do Work (o método `abrirChat()` era órfão, sem botão). **(b)** **Removido o mock** `ChatPrestadorComponent` + rota `servicos/chat/:id`; tirados os botões de chat que levavam ao mock/stub em `detalhes-prestador` e `aprovar-orcamento`. O chat real continua onde há Work (`detalhes-solicitacao` cliente + `detalhes-trabalho` fornecedor). **Chat pré-aceite** (no orçamento) depende de **BE-CHAT-1**. |
| **E2E-1a/1b (Fatia 5)** | **Category-first** (decisão de IA em `auditoria-ux-jornada-e2e.md`): a **`Atividade` virou índice por categoria** (lista só categorias com atividade + contador → abre a tela da categoria), em vez de feed único. Nova tela **`/servicos/atividade`** une **Orçamentos + Trabalhos** com abas **Ativos / Histórico** (garantia/ações no detalhe). Card de serviços da home passa a apontar para `/servicos/atividade`. Índice cobre **Serviços + Delivery**; demais verticais entram depois (E2E-1c). |
| **E2E-2a (Fatia 6)** | **Urgência na entrada do fornecedor**: o hub (`/fornecedor`) passa a carregar e mostrar **cards de pendência separados** (Q-E2E-3) no topo — **"trabalho aprovado — inicie"** (Work `Received`/`Pending`) e **"orçamentos a responder (N)"** (budget `Received`/`Pending`), cada um levando ao lugar certo. Só front. "Nova mensagem" fica para **BE-Q5**. |
| **E2E-1c (Fatia 7)** | Índice de **Atividade** do cliente completado com as **7 verticais**: além de Serviços/Delivery, entram **Compra e Venda** (`negociacoes`), **Aluguel** (`alugueis`), **Transporte** (`pedidos`), **Hospedagem** (`reservas`) e **Empregos** (`minhasCandidaturas`), com contador de **ativos** (predicado por status de cada vertical) → link para a lista própria. Só aparece categoria com atividade (Q-E2E-1); tolerante a verticais sem back (retornam vazio). Só front. |
